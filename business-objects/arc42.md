...
Überall im Editor — in Freitext, innerhalb von Mermaid-, PlantUML- und Draw.io-Codeblöcken — löst das Tippen von `[[` ein Autocomplete-Dropdown aus, das live gegen das Entity-Repository sucht. Der Nutzer tippt weiter, um das Ergebnis einzuschränken; mit Enter oder Klick wird die Entität eingesetzt.

`[[` (doppelte eckige Klammer) ist als Trigger eindeutig: es erscheint weder in URLs, noch in E-Mail-Adressen, noch in Mermaid/PlantUML/Draw.io-Syntax — null False-Positives. Bekannt aus Obsidian, Logseq, Confluence als „Link zu einem Objekt".
...
| Draw.io Shape-Label | `[[CRM` → Auswahl | `CRM-System` (reiner Text im Shape-Label) |

Im **Markdown-Freitext** wird `[[Name|entity:ID]]` gespeichert. Beim Rendering löst das System den aktuellen Namen via ID auf — wird die Entität umbenannt, zeigt der Badge automatisch den neuen Namen. Klick öffnet die Entitäts-Detailansicht.

In **Mermaid-, PlantUML- und Draw.io-Blöcken** kann kein strukturierter Link eingebettet werden. Das Autocomplete setzt dort den Namen als reinen Text ein. Der Bezug ist dadurch lesefreundlich, aber nicht ID-stabil — eine Umbenennung im Repo wird nicht automatisch nachgezogen.
...
| BR-07 | Entity-Mentions im Markdown-Freitext werden im Format `[[Name\|entity:ID]]` gespeichert; beim Rendering MUSS das System den aktuellen `name`-Wert der referenzierten Entität via ID auflösen — bei gelöschter Entität wird `[gelöscht|entity:ID]` in roter Schrift angezeigt | onRead | – |
| BR-08 | Entity-Mentions innerhalb von Mermaid-, PlantUML- und Draw.io-Codeblöcken werden als reiner Textname gespeichert; keine ID-Auflösung beim Rendering (Code-Syntax erlaubt keine OEA-spezifischen Links) | onCreate, onUpdate | – |
| BR-09 | Draw.io-Codeblöcke (` ```drawio `) enthalten valides `<mxGraphModel>`-XML; ungültige XML-Struktur wird vor dem Speichern als Fehler markiert; vor dem Rendering MUSS das XML serverseitig oder via DOMParser sanitisiert werden (keine `<script>`-Tags, keine `javascript:`-URIs in `href`-Attributen) | onSave, onRead | – |
...
| 0.3.0 | 2026-06-26 | Business Engineer | Trigger `/@` → `[[`; Speicherformat `[[Name\|entity:ID]]`; Draw.io als neues Format ergänzt (Formats-Tabelle, Entity-Mention-Tabelle, BR-09) |
